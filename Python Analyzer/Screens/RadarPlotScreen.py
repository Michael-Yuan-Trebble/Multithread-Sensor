...
class RadarPlotScreen(QWidget):
    ...
    def initUI(self):
        ...
        RadarType = self.data[self.data["sensor type"] == "Radar"]

        self.figure = plot.figure(figsize=(8,6))
        ax = self.figure.add_subplot(111,projection="3d")

        sc = ax.scatter(RadarType["x"], RadarType["y"], RadarType["z"],color = "red",label="Radar")

        ax.set_title("Radar Sensor Positions (3D)")
        ax.set_xlabel("X")
        ax.set_ylabel("Y")
        ax.set_zlabel("Z")
        ax.legend()
        ...
        def on_add(sel):
            ...
            sel.annotation.set_text(
                f"({RadarType['x'].iloc[i]}, {RadarType['y'].iloc[i]}, {RadarType['z'].iloc[i]})"
            )
